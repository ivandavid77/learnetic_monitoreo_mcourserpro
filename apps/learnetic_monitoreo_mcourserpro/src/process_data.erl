...
-spec(handle_cast(Request :: term(), State :: #state{}) ->
    ...
    {noreply, NewState :: #state{}, timeout() | hibernate} |
    {stop, Reason :: term(), NewState :: #state{}}).
handle_cast({total_users, Users}, state#{users_received=UsersReceived} = State) ->
    case remove_users_from_total_users(UsersReceived, Users) of
        [] ->
            exit(erlang:whereis(worker_sup), normal),
            fill_tableau_tables(),
            {noreply, #state{}};
        UsersNotReceived ->
            {noreply, state#{users_received=[], total_users=UsersNotReceived, waiting_for_users = true}}
    end;
handle_cast({user_received, User}, #state{users_received=Users, waiting_for_users=false} = State) ->
    {noreply, State#state{users_received = [User|Users]}};
handle_cast({user_received, User}, #state{total_users=Users, waiting_for_users=true} = State) ->
